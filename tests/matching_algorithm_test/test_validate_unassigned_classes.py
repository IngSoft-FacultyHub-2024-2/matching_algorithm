...
from src.matching_algorithm.quality_assurance import validate_unassigned_classes, check_solution
...
class TestValidateUnassignedClasses(unittest.TestCase):
    ...
    def test_partially_unassigned_classes(self):
        ...
        validation_issues = validate_unassigned_classes(teachers, classes, result, [], partially_unassigned)
        self.assertEqual(len(validation_issues), 1)
        issue = validation_issues[0]
        self.assertEqual(issue['class_name'], 'class2')
        self.assertEqual(issue['role'], 'Theory')
    ...
    def test_no_issues_all_classes_assigned(self):
        ...
        result['class2']['Theory'] = ['teacher2']
        validation_issues = validate_unassigned_classes(teachers, classes, result, [], [])
        self.assertEqual(len(validation_issues), 0)
    ...
    def test_no_issues_no_one_can_teach_because_subject_he_know(self):
        ...
        validation_issues = validate_unassigned_classes(teachers_without_teacher2, classes, result, unassigned, partially_unassigned)
        self.assertEqual(len(validation_issues), 0)
