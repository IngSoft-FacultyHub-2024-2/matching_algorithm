...
def solve_timetable(teachers: dict[str, TeacherModel], classes: dict[str, ClassModel]):
            ...
            for subclass in class_info["subClasses"]:
                for i in range(subclass["num_teachers"]):
                    assignments[(teacher, class_name, subclass["role"], i)] = (
                        model.NewBoolVar(
                            f'{teacher}_{class_name}_{subclass["role"]}_{i}'
                        )
                    )
            ...
            for teacher, teacher_info in teachers.items():
                can_teach_info = next(
                    (
                        ...
                        if subject["subject"] == class_info["subject"]
                        and subclass["role"] in subject["role"]
                    ),
                    None,
                )
                ...
                if not can_teach_info:
                    for i in range(subclass["num_teachers"]):
                        model.Add(
                            assignments[(teacher, class_name, subclass["role"], i)] == 0
                        )
                    continue
                    ...
                    for time in times:
                        if time not in teacher_info["available_times"].get(day, []):
                            for i in range(subclass["num_teachers"]):
                                model.Add(
                                    assignments[
                                        (teacher, class_name, subclass["role"], i)
                                    ]
                                    == 0
                                )
                            is_available = False
        ...
        for day in ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday"]:
            for time in range(24):  # Assuming 24-hour clock
                ...
                if conflicting_subclasses:
                    model.Add(
                        sum(
                            assignments[(teacher, class_name, role, i)]
    ...
    # Group preference
    group_preference = model.NewIntVar(
        0, len(teachers) * len(classes) * 2, "group_preference"
    )
    group_matches = []
        ...
        for group in teacher_info.get("groups", []):
            for class_name, class_info in classes.items():
                if class_info["subject"] == group["subject"]:
                    ...
                    group_match = model.NewBoolVar(
                        f"group_match_{teacher}_{class_name}"
                    )
                    ...
                    for other_teacher_info in group["other_teacher"]:
                        ...
                        if other_teacher_assignments:
                            ...
                            model.Add(
                                sum(other_teacher_assignments) >= 1
                            ).OnlyEnforceIf(other_teacher_role_assignments[-1])
                            model.Add(
                                sum(other_teacher_assignments) == 0
                            ).OnlyEnforceIf(other_teacher_role_assignments[-1].Not())
                    ...
                    if teacher_role_assignments and other_teacher_role_assignments:
                        teacher_assigned = model.NewBoolVar(
                            f"teacher_{teacher}_assigned_{class_name}"
                        )
    ...
    if status == cp_model.OPTIMAL or status == cp_model.FEASIBLE:
        ...
        teachers_without_classes = [
            teacher for teacher in teachers if not solver.Value(has_any_class[teacher])
        ]
