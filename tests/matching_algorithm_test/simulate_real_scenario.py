...
class TeachersGenerator:
    ...
    def generate_available_times(self) -> dict:
        ...
        for day in days:
            if random.choice(
                [True, True, False]
            ):  # Teacher may or may not be available on this day
                available_times[day] = sorted(random.sample(hours, random.randint(1, 6)))
        return available_times

    def create_symmetric_groups(
        self, teachers: dict[str, dict], teacher_name: str, other_teacher_name: str
    ) -> None:
...
class ClassesGenerator:
    ...
    def generate_sub_classes(self, subject_roles: list[str]) -> list:
        ...
        for role in subject_roles:
            ...
            sub_class = {
                "role": role,
                "times": {
                    day: hours for day, hours in class_times.items() if hours
                },  # Only keep non-empty days
    ...
    def create_classes(self, num_classes: int) -> dict:
        ...
        for i in range(1, num_classes + 1):
            ...
            classes[class_name] = {"subject": subject_info["subject"], "subClasses": sub_classes}
...
if __name__ == "__main__":
    ...
    assert not are_conflicts(
        assignments.matches, teachers, classes
    ), "Error, there are conflicts in the timetable"
